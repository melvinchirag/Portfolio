...
| [iyinchao/liquid-glass-studio](https://github.com/iyinchao/liquid-glass-studio) | **MIT** — free to use | Studied the real-time refraction/dispersion/fresnel/glare **physics** (raw WebGL2, SDF rounded-rect + Snell's-law shader) and **ported it into React Three Fiber** — SDF merge, Snell's-law refraction, RGB dispersion, Fresnel, glare, over a 2-pass Gaussian-blurred scene capture. Parameterized directly from Melvin's own Liquid Glass Studio export (`liquid-glass-2026-07-27T22-12-22.json`). An earlier attempt used a CSS/SVG `feDisplacementMap` filter instead of a real port — Melvin judged it not good enough ("the liquid glass is shit"); that CSS approach is now superseded dead code. | `site/src/components/scene/LiquidGlassField.tsx` (live); `site/src/components/GlassFilterDefs.tsx` (superseded, unmounted) |
